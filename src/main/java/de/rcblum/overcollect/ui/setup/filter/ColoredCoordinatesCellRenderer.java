...
public class ColoredCoordinatesCellRenderer extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
		...
		if (column > 1) {
			try {
				c.setBackground(new Color((Integer) model.getValueAt(row, 2), (Integer) model.getValueAt(row, 3),
						(Integer) model.getValueAt(row, 4)));
				int val = (Integer) model.getValueAt(row, 2) + (Integer) model.getValueAt(row, 3)
						+ (Integer) model.getValueAt(row, 4);
				if (val < 200)
